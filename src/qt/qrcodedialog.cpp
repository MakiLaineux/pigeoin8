...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non PGN unit input in lnReqAmount, we generate the URI with PGN as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(PigeoinUnits::format(PigeoinUnits::PGN, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
