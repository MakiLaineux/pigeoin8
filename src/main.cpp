...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uPGN
...
{
    // MKL : mining reward is 1 for the first 30 000 blocks, afterwards it is 0. No halving
...
static const int64 nTargetTimespan = 30 * 60; // MKL : adjust difficulty every 30 minutes (10 blocks)
static const int64 nTargetSpacing = 3 * 60; // MKL : 1 block every 3 minutes
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Peu importe";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN; // MKL
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PigeoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PigeoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PigeoinMiner(CWallet *pwallet)
{
    printf("PigeoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pigeoin-miner");
    ...
    try { loop {
        ...
        printf("Running PigeoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PigeoinMiner terminated\n");
        throw;
    }
}

void GeneratePigeoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PigeoinMiner, pwallet));
}
